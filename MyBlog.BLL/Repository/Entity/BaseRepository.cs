   ...
   public class BaseRepository<T> where T : BaseEntity
    {
        ...
        {
            item.Status = MyBlog.DAL.ORM.Enum.Status.Active;
            table.Add(item);
            Save();
        }
        ...
        {
            return table.Where(x => x.Status == MyBlog.DAL.ORM.Enum.Status.Active || x.Status == MyBlog.DAL.ORM.Enum.Status.Modified).ToList();
        }
        ...
        {
            T item = GetById(id);
            item.Status = MyBlog.DAL.ORM.Enum.Status.Deleted;
            Update(item);
        }
